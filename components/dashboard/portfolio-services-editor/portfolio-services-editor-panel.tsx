...
import { PortfolioServiceItemCard } from "@/components/dashboard/portfolio-services-editor/components/portfolio-service-item-card";
import { usePortfolioServicesEditor } from "@/components/dashboard/portfolio-services-editor/hooks/use-portfolio-services-editor";
...
export function PortfolioServicesEditorPanel({ landing }: { landing: Landing }) {
  const {
    addItem,
    expandedItemId,
    items,
    ...
    toggleItem,
    updateItem,
  } = usePortfolioServicesEditor(landing);

  return (
    <section className="space-y-5 py-unit-lg">
      <div className="flex items-start justify-between gap-3">
        <EditorSectionTitle
          description={PORTFOLIO_SERVICES_EDITOR_COPY.description}
          title={PORTFOLIO_SERVICES_EDITOR_COPY.title}
        />
        <button
          aria-label={PORTFOLIO_SERVICES_EDITOR_COPY.addItemAriaLabel}
      ...
      >
        <Plus aria-hidden className="size-4" />
        {PORTFOLIO_SERVICES_EDITOR_COPY.addItem}
      </button>
    </section>
  );
}
