...
import { PortfolioProjectItemCard } from "@/components/dashboard/portfolio-projects-editor/components/portfolio-project-item-card";
import { usePortfolioProjectsEditor } from "@/components/dashboard/portfolio-projects-editor/hooks/use-portfolio-projects-editor";
...
export function PortfolioProjectsEditorPanel({ landing }: { landing: Landing }) {
  const {
    addItem,
    editProjectPage,
    expandedItemId,
    ...
    toggleItem,
    updateItem,
  } = usePortfolioProjectsEditor(landing);

  return (
    <section className="space-y-5 py-unit-lg">
      <div className="flex items-start justify-between gap-3">
        <EditorSectionTitle
          description={PORTFOLIO_PROJECTS_EDITOR_COPY.description}
          title={PORTFOLIO_PROJECTS_EDITOR_COPY.title}
        />
        <button
          aria-label={PORTFOLIO_PROJECTS_EDITOR_COPY.addProjectAriaLabel}
      ...
      <div className="space-y-3">
        {items.map((item, index) => (
          <PortfolioProjectItemCard
            assetName={getAssetName(item.image)}
            ...
            onEditPage={() => editProjectPage(item.id)}
      ...
      >
        <Plus aria-hidden className="size-4" />
        {PORTFOLIO_PROJECTS_EDITOR_COPY.addProject}
      </button>
    </section>
  );
}
