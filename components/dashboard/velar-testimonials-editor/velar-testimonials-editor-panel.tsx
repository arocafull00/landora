...
import { VelarTestimonialItemCard } from "@/components/dashboard/velar-testimonials-editor/components/velar-testimonial-item-card";
import { useVelarTestimonialsEditor } from "@/components/dashboard/velar-testimonials-editor/hooks/use-velar-testimonials-editor";
...
export function VelarTestimonialsEditorPanel({ landing }: { landing: Landing }) {
  const {
    addItem,
    expandedItemId,
    items,
    ...
    toggleItem,
    updateItem,
  } = useVelarTestimonialsEditor(landing);

  return (
    <section className="space-y-5 py-unit-lg">
      <div className="flex items-start justify-between gap-3">
        <EditorSectionTitle
          description={VELAR_TESTIMONIALS_EDITOR_COPY.description}
          title={VELAR_TESTIMONIALS_EDITOR_COPY.title}
        />
        <button
          aria-label={VELAR_TESTIMONIALS_EDITOR_COPY.addItemAriaLabel}
      ...
      >
        <Plus aria-hidden className="size-4" />
        {VELAR_TESTIMONIALS_EDITOR_COPY.addItem}
      </button>
    </section>
  );
}
