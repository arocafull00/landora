...
export function FloristeriaTestimonialsSection({ content }: { content: LandingContent }) {
  ...
  return (
    <section id="testimonios" className="scroll-mt-24 bg-[#2D5016] px-6 py-24 md:px-10 md:py-32 lg:px-16">
      <div className="mx-auto max-w-6xl">
        <div className="mb-16 border-b border-white/10 pb-12" data-aos="fade-up">
          <span
            aria-hidden
            className="mb-6 block text-[clamp(64px,10vw,96px)] leading-none text-white/30"
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex gap-0.5">
              {Array.from({ length: featured.rating }).map((_, i) => (
                <Star className="h-4 w-4 fill-white text-white" key={i} />
              ))}
            </div>
            <p className="text-sm font-semibold text-white">{featured.author}</p>
            {featured.verified && (
              <span className="text-xs text-white/70">Verificado</span>
            )}
          </div>
        </div>
              ...
              <div className="border-t border-white/10 pt-6" key={testimonial.id}>
                <div className="mb-3 flex gap-0.5">
                  {Array.from({ length: testimonial.rating }).map((_, i) => (
                    <Star className="h-3.5 w-3.5 fill-white text-white" key={i} />
                  ))}
                </div>
