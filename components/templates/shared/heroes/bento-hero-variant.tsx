  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100dvh-8rem)] w-full max-w-7xl items-center gap-14 py-12 lg:grid-cols-[0.88fr_1.12fr] lg:gap-16 lg:py-16">
        ...
        >
          {hero.eyebrow ? (
            <p className="mb-5 flex items-center gap-3 font-body text-xs font-semibold uppercase tracking-[0.18em] text-[var(--site-primary)]">
              <span aria-hidden className="h-px w-8 bg-[var(--site-primary)]" />
              {hero.eyebrow}
            </p>
          ) : null}
          <h1 className="text-balance font-headline text-[clamp(3rem,6vw,5.5rem)] font-bold leading-[0.96] tracking-[-0.06em]">
            {hero.title}
            {hero.subtitle ? (
          ...
          {hero.description ? (
            <p className="mt-6 max-w-xl text-pretty font-body text-lg leading-relaxed text-[var(--site-text-muted)]">
              {hero.description}
            </p>
          ) : null}
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <a
              className="inline-flex min-h-13 items-center justify-center gap-2 rounded-xl bg-[var(--site-primary)] px-6 py-3 font-body text-sm font-bold text-[var(--site-on-primary)] shadow-xl shadow-[var(--site-primary)]/20 transition-[transform,background-color] hover:-translate-y-0.5 hover:bg-[var(--site-primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--site-primary)] focus-visible:ring-offset-2"
              href={primaryCtaHref}
            ...
            <a
              className="inline-flex min-h-13 items-center justify-center rounded-xl border border-[var(--site-border)] px-6 py-3 font-body text-sm font-bold text-[var(--site-text)] transition-[transform,background-color,border-color] hover:-translate-y-0.5 hover:border-[var(--site-primary)] hover:bg-[var(--site-surface-alt)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--site-primary)]"
          ...
          {stats.length > 0 ? (
            <div className="mt-9 flex flex-wrap gap-x-7 gap-y-4 text-xs text-[var(--site-text-muted)]">
              {stats[0] ? <p><strong className="block text-lg text-[var(--site-text)]">{stats[0].value}</strong>{stats[0].label}</p> : null}
              {stats[1] ? <p><strong className="block text-lg text-[var(--site-text)]">{stats[1].value}</strong>{stats[1].label}</p> : null}
              {stats[2] ? <p><strong className="block text-lg text-[var(--site-text)]">{stats[2].value}</strong>{stats[2].label}</p> : null}
            </div>
          ) : null}
        </div>
        ...
        >
          ...
          <div aria-hidden className="flex flex-col justify-between rounded-[1.4rem] bg-[var(--site-primary)] p-5 text-[var(--site-on-primary)] shadow-xl sm:p-6">
            <span className="text-sm opacity-70">{stats[0]?.label || BENTO_COPY.metricLabel}</span>
            <strong className="font-headline text-4xl tracking-[-0.06em] sm:text-5xl">{stats[0]?.value || BENTO_COPY.metricValue}</strong>
          </div>
          <div aria-hidden className="rounded-[1.4rem] border border-[var(--site-border)] bg-[var(--site-surface)]/80 p-5 shadow-xl backdrop-blur-md sm:p-6">
            <strong className="font-body text-sm">{BENTO_COPY.chartLabel}</strong>
