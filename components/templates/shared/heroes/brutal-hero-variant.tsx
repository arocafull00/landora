  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100dvh-8rem)] w-full max-w-7xl items-end gap-10 py-12 lg:grid-cols-[1fr_19rem] lg:py-16">
        <div
        >
          <div className="mb-5 flex items-center justify-between border-b-2 border-[var(--site-dark)] pb-4 font-body font-semibold uppercase tracking-[0.12em] text-site-content">
            <span>{hero.eyebrow || BRUTAL_COPY.location}</span>
            <span>{content.brand}</span>
          </div>
          <h1 className="max-w-6xl text-balance font-headline font-extrabold uppercase leading-[0.76] tracking-[-0.085em] text-site-title-lg">
            {hero.title}
            {hero.subtitle ? (
        ...
        >
          {hero.description ? (
            <p className="text-pretty font-body font-medium leading-relaxed text-site-subtitle">
              {hero.description}
            </p>
          ) : null}
          <a
            className="mt-7 inline-flex min-h-13 w-full items-center justify-center gap-2 bg-[var(--site-dark)] px-5 py-3 font-body font-bold uppercase tracking-wide text-[var(--site-accent)] transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--site-dark)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--site-accent)] text-site-button"
            href={primaryCtaHref}
      ...
      <div className="absolute inset-x-0 bottom-0 overflow-hidden border-t-2 border-[var(--site-dark)] bg-[var(--site-dark)] py-2.5 text-[var(--site-accent)]">
        <p
          className="w-max whitespace-nowrap font-body font-semibold uppercase tracking-[0.12em] text-site-content"
        >
