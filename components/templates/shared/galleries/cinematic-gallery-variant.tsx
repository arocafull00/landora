      ...
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[var(--site-dark)] py-8 text-[var(--site-on-dark)] shadow-[0_24px_70px_color-mix(in_srgb,var(--site-dark)_18%,transparent)] sm:py-10">
        <div className="flex flex-col items-start justify-between gap-7 px-5 sm:px-8 lg:flex-row lg:items-end lg:px-10">
          <div className="max-w-4xl">
            <span className="inline-flex rounded-full bg-[var(--site-on-dark)]/10 px-3 py-2 font-mono text-xs font-medium uppercase tracking-[0.08em] text-[var(--site-accent)]">
              Cinematic scroll
            </span>
            <h2 className="mt-4 max-w-3xl text-balance font-heading text-4xl font-bold leading-[0.98] tracking-[-0.04em] sm:text-5xl lg:text-6xl">
              {heading.title}
            </h2>
            {heading.subtitle ? (
              <p className="mt-4 max-w-2xl text-pretty font-body text-base leading-relaxed text-[var(--site-on-dark)]/65 sm:text-lg">
                {heading.subtitle}
              </p>
            ) : null}
          </div>
